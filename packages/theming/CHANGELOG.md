...
- 🏗️ Refactored to `ThemeModule.forRoot()` / `forFeature()` DynamicModule pattern
...
- 📁 Added `constants/` folder with `tokens.constant.ts` and `themes.constant.ts`
...
- 🌙 `useColorMode()` hook — `{ mode, setMode, resolvedMode, isDark, isLight, toggle }`
- 📦 `ThemeModule.registerTheme()` and `ThemeModule.registerCustomizer()` convenience methods
...
- 🎉 Initial release of @abdokouta/react-theming
