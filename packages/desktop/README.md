...
Platform-agnostic Electron desktop integration with DI, decorators, and 23 injectable services.
...
- 23 injectable services across 7 domains (POS hardware, offline/sync, window management, security, system integration, updates, diagnostics)
- Platform abstraction — ElectronBridge (real IPC) or BrowserBridge (graceful fallbacks)
- @Menu() / @MenuItem() decorators for native Electron menus
- Kbd integration — menu shortcuts auto-register with ShortcutRegistry
- Generic IPC — services use `bridge.invoke('channel', ...args)`, no preload changes needed
